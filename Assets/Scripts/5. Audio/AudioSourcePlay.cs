...
public class AudioSourcePlay : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        // 첫 번째 버튼은 3D 사운드 재생 버튼입니다.
        // 버튼이 클릭되면 mAudioSource.Play() 함수를 호출하여 AudioSource를 통해 연결된 오디오 파일을 재생합니다.
        if(GUI.Button(new Rect(Screen.width * 0.1f, Screen.height * 0.1f, Screen.width * 0.2f, Screen.height * 0.1f), "3D 사운드 재생"))
        {
            mAudioSource.Play();
        }

        // 두 번째 버튼은 3D 사운드 멈춤 버튼입니다.
        // 버튼이 클릭되면 mAudioSource.Stop() 함수를 호출하여 현재 재생 중인 오디오를 멈춥니다.
        if(GUI.Button(new Rect(Screen.width * 0.1f, Screen.height * 0.3f, Screen.width * 0.2f, Screen.height * 0.1f), "3D 사운드 멈춤"))
        {
            mAudioSource.Stop();
        }        
    }
